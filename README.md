...
GitHub can generate these packages automatically. Push the `0.1.0` RC tag; the
release workflow runs checks, builds the archives, uploads workflow artifacts,
and creates a GitHub prerelease for the `0.1.0` tag.
...
git tag -a 0.1.0 -m "KOReader Git Sync 0.1.0 release candidate"
git push origin 0.1.0
```

Pushing the `0.1.0` tag triggers the GitHub release workflow and publishes it as
a GitHub prerelease with the generated zip and tarball attached.
